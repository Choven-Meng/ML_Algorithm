...
泛化误差的公式可以理解为模型的预测错误率，有两部分组成：一部分是由于模型太简单而带来的估计不准确的部分(bias)，另一部分是由于模型太复杂带来的更大的变化空间和不确定性(Variance).
...
如果是小训练集，高偏差/低方差的分类器(例如NB)要比低偏差/高方差(如KNN)的分类的优势大，因为后者会过拟合。但是，随着训练集的增长，模型对于原数据集的预测能力会越来越好，偏差就会降低，此时低偏差/高方差分类器就会渐渐表现出其优势，因为他们有较低的渐近误差。([朴素贝叶斯是高偏差低方差模型的原因](https://blog.csdn.net/u014365862/article/details/52937983))
...
朴素贝叶斯假设各个数据之间是无关的，是一个被严重简化了的模型，所以对于这样一个简单模型，大部分场合都会bias部分大于variance部分，就是高偏差低方差。



**优点:**      
* 对小规模的数据表现很好，适合多分类任务，适合增量式训练。     
* 对**缺失数据不敏感**，算法也比较简单，常用于文本分类。
...
* 便利的观测样本概率分数；  
* 对逻辑回归而言，多重共线性不是问题，可以结合L2正则化来解决该问题。   
 
**缺点：**   
* 当特征空间很大时，逻辑回归的性能不好；  
* 容易欠拟合，一般准确率不高；   
* 不能很好的处理大量多类特征或变量；只能处理两分类问题(在此基础上衍生出的softmax可以用于多分类)，且必须线性可分；  
* 对于非线性特征，需要进行转换。
...
如何选择一个最佳的k值取决于数据。一般情况下，在**分类时较大的K值能够减小噪声的影响**，但会使类别之间的界限变得模糊。一个较好的K值可通过各种启发式技术来获取，比如，交叉验证。另外**噪声和非相关性特征向量的存在会使K近邻算法的准确性减小**。   
...
**优点：**   
> * adaboost是一个有很高精度的分类器。  
> * 可以通过各种方法构建子分类器，adaboost算法提供的是框架。  
> * 简单，**不用做特征筛选**。  
> * 不容易发生过拟合。  
...
> * K值比较难以选取。  
> * 对初值的簇心值敏感，对于不同的初始值，可能会有不同的结果。  
> * 不适合发现非凸面形状的簇，或者大小差别很大的簇。  
> * 对于**噪声和孤立点非常敏感**。
...
> * 对**噪声神经有较高的鲁棒性和容错能力**，能充分逼近复杂的非线性关系。  
> * 具有联想记忆的能力。

**缺点：**   
> * 需要大量的参数，如网络拓扑结构、权值和阈值等。  
> * 不能观察之前的学习过程，输出结果难以解释，会影响到结果的可信度和可接受程度。   
> * 学习时间过长。
